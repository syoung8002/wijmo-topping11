<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-title v-if="value._links">
            Department # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            Department
        </v-card-title >
...
export default {
    ...
    created(){
        ...
        websocketUrl.pathname = "/rsocket/departments";
        websocketUrl.hash = "";
        ...
        client.connect().subscribe({
            ...
            const endpoint = "departments."+ me.value.id +".get"
    ...
    methods: {
        ...
        async save(){
            try {
                ...
                if(!this.offline) {
                    if(this.isNew) {
                        temp = await axios.post(axios.fixUrl('/departments'), this.value)
                    } else {
