<template>
    <div>
        ...
        <h1 style = "position:absolute; left:23px; top:15px;">Department</h1>
        ...
        <v-row>
            <Department :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            async search(query) {
                ...
                me.values = temp.data._embedded.departments;
